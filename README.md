...
* Safe/passive mode is on by default. It blocks literal/private/local destinations and DigiScope never performs active port scans, internet-wide scanning or hidden-service crawling. Explicit **Authorized same-host crawl** and **Authorized DNS wordlist enumeration** opt-ins are available for assets you own: DNS enumeration uses a bundled 1,200-label list or a per-scan custom list, wildcard filtering, a candidate cap and request delay. The web mapper obeys robots when available, uses GET only, stays on one host, skips forms/binaries/query strings, extracts optional public contact signals, and stores metadata rather than page bodies.
